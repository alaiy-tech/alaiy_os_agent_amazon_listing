// Run Amazon Agent — the Amazon listing agent's own desk page: the Auto/Manual
// mode toggle, the enrich flow (idle -> loading -> done), the loader, and the
// side-by-side Original/Generated comparison.
//
// Auto mode's enrich flow calls the real agent engine (alaiy_os.api.agents.
// run_agent/get_run) — it reads the selected listing's actual data and photos (or
// the given image_url) and the "Generated Listing" panel shows genuine AI output,
// polled until the run finishes. The "Original Listing" panel is populated from a
// plain frappe.db.get_value on the same Amazon Listing, fetched alongside the run,
// so "Before" reflects what's actually in the DB rather than placeholder text.
// Manual mode stays a plain form with no agent involvement at all — that is by
...
// tokens (--s-*, see run_amazon_agent.css) rather than literal hex values and
// Google Fonts — same design system as the rest of Alaiy OS.
...
frappe.pages["run-amazon-agent"].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: "Run Amazon Agent",
		single_column: true,
	});
...
frappe.pages["run-amazon-agent"].on_page_show = function () {
...
class RunAgentPage {
	...
	render_auto() {
			...
			this.$auto_body.html(`
				<div class="ra-callout ra-callout--error">
					<div class="ra-callout-title">
						${frappe.utils.icon("triangle-alert", "sm")} ${__("No Amazon listing agent is available")}
					</div>
					<p>${__(
						"The Amazon listing agent is not registered or is disabled on this site. Check OS Agent Registry, or run a migrate to register it."
					)}</p>
				</div>
			`);
			return;
			...
			<div class="ra-card ra-enrich-card frappe-card">
				<div class="ra-card-header">
					...
					<div class="ra-card-header-text">
						...
						<p>${__("Fill in what you have, then generate an Amazon-ready listing.")}</p>
					</div>
				</div>

				<div class="ra-info-strip">
					${frappe.utils.icon("info", "xs")}
					<span>${__("Provide at least one: SKU or Image URL.")}</span>
				</div>
				...
				<div class="ra-bordered-block">
					<div class="ra-field-grid">
						<div class="ra-field-group">
							<label>${__("SKU")}</label>
							<div class="ra-input-wrap" id="ra-sku-wrap"></div>
							<p class="ra-field-help">${__("An existing Amazon listing — reads its data, issues and photos.")}</p>
						</div>
						<div class="ra-field-group">
							...
							<p class="ra-field-help">${__("No listing yet? Paste a photo URL instead.")}</p>
						</div>
					</div>
				</div>
		...
		// Real Link control (searchable dropdown against the actual Amazon Listing
		// records in the DB) rather than a plain text input the user would have to
		// type an exact SKU into blind.
		this.sku_control = frappe.ui.form.make_control({
			...
			parent: this.$auto_body.find("#ra-sku-wrap")[0],
			only_input: true,
			render_input: true,
		});
		this.sku_control.refresh();
		...
		// Pick up a SKU pre-selected by the Item form's Enrich button on the very
		// first build (on_page_show handles subsequent visits).
		this.apply_route_options();
	}

	// Prefill from frappe.route_options (set by public/js/item_enrich.js before
	// routing here). Switches to Auto mode and selects the SKU; the user still
	// clicks Enrich themselves.
	//
	// Auto mode renders only once the agent has loaded, so a route option can
	// land before the SKU control exists. It is stashed rather than dropped, and
	// render_auto replays it — otherwise arriving from the Item form would silently
	// lose the listing on a cold page.
	apply_route_options() {
		const opts = frappe.route_options;
		if (opts && opts.sku) {
			frappe.route_options = null;
			this.pending_sku = opts.sku;
		}
		if (!this.pending_sku) return;

		if (this.state.mode !== "auto") {
			this.$root.find('.ra-mode-btn[data-mode="auto"]').trigger("click");
		}
		if (this.sku_control) {
			this.sku_control.set_value(this.pending_sku);
			this.pending_sku = null;
		}
	}
	...
	on_enrich_click() {
		...
		if (this.state.enrichState === "done") {
			...
			// clicking Enrich again starts a fresh loading pass.
		...
		const sku = this.sku_control.get_value() || "";
		const image_url = (this.$image_url.val() || "").trim();
		if (!sku && !image_url) {
			frappe.msgprint({
				title: __("Missing Input"),
				message: __("Provide at least a SKU or an Image URL before running the agent."),
				indicator: "orange",
			});
			return;
		}
	...
	start_enrich(inputs) {
		...
		// — this doesn't block/slow the actual enrichment. Read from the Amazon
		// Listing, the same doctype the agent reads and writes back onto, so
	...
	render_loader() {
		...
		const sub = Object.values(this.state.options).some(Boolean)
			? __("Reading listing data and photos, working on images")
			: __("Reading listing data and photos");
		this.$flow_area.removeClass("is-visible").html(`
	...
	render_comparison(inputs, listing) {
		...
		const orig = this.original_listing || {};
		const orig_bullets = this.original_bullets || [];

		const confidence_color = { high: "green", medium: "orange", low: "red" }[listing.confidence] || "gray";
		const marketplace_html = orig.marketplace
			? `<span class="indicator-pill blue">${esc(orig.marketplace)}</span>`
			: "";
		...
		this.$flow_area.removeClass("is-visible").html(`
			<div class="ra-comparison-grid">
				<div class="ra-compare-card ra-compare-card--original ${this.state.origCollapsed ? "is-collapsed" : ""}">
					...
					<div class="ra-compare-body">
						<div class="ra-field-block">
							...
							<div class="ra-static-value">${orig.title ? esc(orig.title) : "—"}</div>
						</div>
						<div class="ra-field-block">
							<div class="control-label">${__("Bullet Points")}</div>
							${bullet_list(orig_bullets)}
						</div>
						<div class="ra-field-block">
							<div class="control-label">${__("Description")}</div>
							<div class="ra-static-value">${
								orig.description ? esc(strip_html(orig.description)) : "—"
							}</div>
						</div>
						<div class="ra-field-block">
							<div class="control-label">${__("ASIN")}</div>
							<div class="ra-static-value">${orig.asin ? esc(orig.asin) : "—"}</div>
						</div>
					</div>
				</div>
				...
				<div class="ra-compare-card ra-compare-card--generated ${this.state.genCollapsed ? "is-collapsed" : ""}">
					...
					<div class="ra-compare-body">
						<div class="ra-listing-badges">${marketplace_html}${confidence_html}</div>

						${needs_review_html}

						<div class="ra-field-block">
							<div class="control-label">${__("Title")} ${title_count}</div>
						...
						<div class="ra-field-block">
							<div class="control-label">${__("Search Keywords")}</div>
							<div class="ra-static-value">${
								(listing.keywords || []).length ? esc((listing.keywords || []).join(", ")) : "—"
							}</div>
						</div>
	...
	render_manual() {
		this.$manual_body.html(`
			<div class="ra-card frappe-card">
				...
				<div class="ra-bordered-block">
					<div class="ra-field-grid">
						<div class="ra-field-group"><label>${__("SKU")}</label><input type="text" class="form-control"></div>
						<div class="ra-field-group"><label>${__("ASIN")}</label><input type="text" class="form-control"></div>
				...
				<div class="ra-bordered-block">
					<div class="ra-field-group ra-field-group--full">
						...
						<textarea class="form-control" rows="5" placeholder="${__("One per line, up to five.")}"></textarea>
					</div>
					<div class="ra-field-group ra-field-group--full">
					...
					<div class="ra-field-group ra-field-group--full">
						...
						<textarea class="form-control" rows="2" placeholder="${__("Comma separated.")}"></textarea>
					</div>
				</div>

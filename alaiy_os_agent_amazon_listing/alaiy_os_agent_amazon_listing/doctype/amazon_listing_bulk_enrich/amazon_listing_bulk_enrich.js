...
frappe.ui.form.on("Amazon Listing Bulk Enrich", {
	refresh(frm) {
		...
		if (in_flight) {
			frm.dashboard.set_headline(
				__("{0} of {1} done — this page updates as listings finish.", [
			...
			// for the amazon_bulk_enrich_done that flips it to its final status.
...
function show_image_progress(frm) {
	...
	frappe
		.xcall("alaiy_os_agent_amazon_listing.api.get_bulk_status", { batch: frm.doc.name })
		.then((r) => {
...
// One listing finishing (or the batch closing) is worth a reload — the rows are
...
function listen(frm) {
	...
	frappe.realtime.on("amazon_bulk_enrich_progress", reload);
	frappe.realtime.on("amazon_bulk_enrich_done", reload);
}

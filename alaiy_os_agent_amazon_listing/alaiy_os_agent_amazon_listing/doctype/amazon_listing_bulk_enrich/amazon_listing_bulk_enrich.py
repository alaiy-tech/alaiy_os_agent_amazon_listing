...
"""One bulk enrichment request: a list of listings and the state of each one's run.
...
from alaiy_os_agent_amazon_listing.bulk import DEFAULT_BATCH_SIZE, enqueue_chunks
...
class AmazonListingBulkEnrich(Document):
	def validate(self):
		self._dedupe_items()
	...
	def _dedupe_items(self):
		"""The same listing twice would mean two runs racing on one enriched listing."""
		seen, kept = set(), []
	...
	@frappe.whitelist()
	def start(self):
		"""Queue every listing in this batch. Returns {batch, items, jobs}."""
		self.check_permission("write")
		if self.status in IN_FLIGHT:
			frappe.throw(f"{self.name} is already {self.status.lower()}.")
		if not self.items:
			frappe.throw("Add at least one listing before starting.")
	...
	def cancel_batch(self):
		"""Stop after the listings already in flight.

		Named `cancel_batch` rather than `cancel` so it does not shadow
		Document.cancel. Each worker re-reads this status before its next listing,
	...
	def retry_failed(self):
		...
		if not rows:
			frappe.throw("Nothing to retry — no failed or cancelled listings.")
